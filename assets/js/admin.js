...
async function loadAdminStats() {
  try {
    ...
    const pending = skillsRes?.data?.filter(s => (s.attributes?.skillStatus || s.skillStatus) === "pending").length || 0;
...
async function loadPendingSkills() {
  ...
  try {
    ...
    el.innerHTML = skills.map(s => {
      ...
      return `
        <div class="flex items-start gap-4 p-4 bg-white border border-slate-100 rounded-2xl" id="pending-row-${sid}">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap gap-2 text-xs">
              ...
              ${levelBadge(a.level)}
              <span class="text-slate-300">·</span>
              <span class="font-medium text-slate-400">by ${a.owner?.username || a.owner?.attributes?.username || "Unknown"}</span>
            </div>
          </div>
          <div class="flex flex-col gap-1.5 shrink-0">
            <button onclick="approveSkill('${sid}')"
            ...
            <button onclick="rejectSkill('${sid}')"
...
async function loadAllSkills() {
  ...
  try {
    ...
    ${skills.map(s => {
      ...
      const cat = a.skillCategory?.name || a.skillCategory?.attributes?.name || "—";
      const sid = s.documentId || s.id;
      return `<tr class="hover:bg-slate-50 transition" id="all-row-${sid}">
            ...
            <td class="py-3 pr-4">${statusBadge(a.skillStatus)}</td>
            <td class="py-3 pr-4 text-slate-500 font-medium">${a.owner?.username || a.owner?.attributes?.username || "-"}</td>
            <td class="py-3">
              <div class="flex gap-1.5">
                ${a.skillStatus !== "approved" ? `<button onclick="approveSkillAll('${sid}')" class="px-2.5 py-1 rounded-xl bg-emerald-100 text-emerald-700 text-xs font-bold hover:bg-emerald-200 transition">Approve</button>` : ""}
                ${a.skillStatus !== "rejected" ? `<button onclick="rejectSkillAll('${sid}')" class="px-2.5 py-1 rounded-xl bg-red-100 text-red-600 text-xs font-bold hover:bg-red-200 transition">Reject</button>` : ""}
              </div>
            </td>
          </tr>`;
    }).join("")}
...
async function loadCategories() {
  ...
  try {
    ...
    el.innerHTML = cats.map(c => {
      ...
      return `
        <div class="flex items-center gap-3 p-4 bg-white border border-slate-100 rounded-2xl" id="cat-row-${cid}">
          ...
          <div class="flex gap-1.5">
            <button onclick="openEditCat('${cid}','${encodeURIComponent(a.name)}','${encodeURIComponent(a.description || '')}','${a.icon || ''}')"
