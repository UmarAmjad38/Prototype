...
async function initDashboard() {
    ...
    await refreshUser(); // Force refresh role/session
    populateNavUser();
    initLogout();
...
async function refreshUser() {
    try {
        ...
        console.error("Dashboard: refreshUser error:", err);
    }
}
...
async function loadCategories() {
    try {
        ...
        allCategories.forEach(c => {
            ...
            opt.value = c.documentId || c.id; // Use documentId for Strapi 5
...
async function loadMySkills() {
    ...
    if (!user || !user.id) {
        container.innerHTML = `<p class="text-center text-sm text-slate-400 py-8">Session error. Please logout and login again.</p>`;
        return;
    }
    ...
    try {
        ...
        <div class="text-center py-14">
          ...
          <p class="text-slate-500 font-black text-sm mb-4">You haven't listed any skills yet.</p>
          <button onclick="openNewSkillModal()" class="px-6 py-2.5 rounded-2xl bg-primary-600 text-white text-sm font-bold hover:bg-primary-700 transition shadow-xl shadow-primary-500/20">
        ...
        const pending = skills.filter(s => {
            const st = s.attributes?.skillStatus || s.skillStatus;
            return st === "pending";
        }).length;
        const approved = skills.filter(s => {
            const st = s.attributes?.skillStatus || s.skillStatus;
            return st === "approved";
        }).length;
        ...
        container.innerHTML = skills.map(s => {
            ...
            const cat = a.skillCategory?.name || a.skillCategory?.attributes?.name || "—";
            const img = a.imageUrls?.[0] || "";
            const status = a.skillStatus || "pending";

            return `
        <div class="flex items-start gap-4 p-4 bg-white border border-slate-100 rounded-2xl hover:border-primary-100 transition skill-card" id="skill-row-${s.documentId || s.id}">
          ...
          <div class="flex gap-2 shrink-0">
            <button onclick="openEditSkill('${s.documentId || s.id}')" title="Edit"
            ...
            <button onclick="confirmDelete('${s.documentId || s.id}','${a.title.replace(/'/g, "\\'")}')" title="Delete"
        ...
        console.error("Dashboard: loadMySkills error:", err);
        container.innerHTML = `
          <div class="text-center py-14">
            <div class="text-4xl mb-3">😕</div>
            <p class="text-slate-500 font-black text-sm mb-1">No skills found.</p>
          </div>`;
    }
}
...
async function openEditSkill(id) {
    ...
    try {
        ...
        // Set category
        const catId = a.skillCategory?.documentId || a.skillCategory?.id || a.skillCategory?.data?.id || "";
    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                skillCategory: form.querySelector("#skill-category").value || null,
                level: form.level.value,
                location: form.location.value.trim(),
                availability: form.availability.value.trim(),
                // owner is handled by the backend controller
                skillStatus: "pending",
            ...
            form.reset();
            document.getElementById("skill-image-preview").innerHTML = "";
            document.getElementById(`skill-row-${editingSkillId}`)?.remove(); // Optimistic remove/reload
            await loadMySkills();
        } catch (err) {
